...
function M.build(pos, tree, strategy)
	...
	logger.debug("Handling file " .. pos_path_filename .. " in " .. pos_path_folderpath)
	...
	for _, golist_item in ipairs(golist_data) do
		if golist_item.TestGoFiles ~= nil then
			...
			then
				...
				logger.debug("Matched " .. golist_item.ImportPath)
				break
			else
				logger.debug("Didn't match " .. golist_item.ImportPath)
			end
		end
	...
	logger.debug("Final package name " .. package_name)
